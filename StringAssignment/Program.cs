...
Console.WriteLine($"In this string are {words.Length} words");


//- Display the sentence count of this string
string[] sentence = text.Split(new char[] { '.','!','?' });
Console.WriteLine($"In this string are {sentence.Length} sentences");
...
Console.WriteLine("\n " + ReverseString(text));//charater by charater
Console.WriteLine("\n " + ReverseStringWords(text));//word by word



//- In the given string, replace all occurances of "object-oriented programming" with "OOP" and display the new string
string RepleceWord (string str,string oldWord,string nowWoed)
{
    return str.Replace(oldWord,nowWoed);
}
string replaced = RepleceWord(text, "object-oriented programming", "OOP");
